...
// 1. Importaciones de Componentes del Sistema
    ...
    <div class="dashboard-container" *ngIf="usuarioLogueado">
      <header>
        ...
        <div class="slider-box">
          ...
          <small *ngIf="enTiempoReal" class="alert-text">Simulación activa ejecutándose a 20 ticks/seg.</small>
          <small *ngIf="!enTiempoReal" class="alert-text manual">Modo manual: Arrastra el slider para controlar los relojes.</small>
        </div>

        <div class="selector-relojes">
          <button [class.active]="relojSeleccionado === 'sol'" (click)="seleccionarReloj('sol')">☀️ 1. Sol (~1500 a.C.)</button>
          <button [class.active]="relojSeleccionado === 'agua'" (click)="seleccionarReloj('agua')">💧 2. Agua (~1400 a.C.)</button>
          <button [class.active]="relojSeleccionado === 'arena'" (click)="seleccionarReloj('arena')">⏳ 3. Arena (~Siglo III d.C.)</button>
          <button [class.active]="relojSeleccionado === 'incienso'" (click)="seleccionarReloj('incienso')">🪵 4. Incienso (~Siglo VI d.C.)</button>
          <button [class.active]="relojSeleccionado === 'vela'" (click)="seleccionarReloj('vela')">🕯️ 5. Vela (~Siglo IX d.C.)</button>
          <button [class.active]="relojSeleccionado === 'aceite'" (click)="seleccionarReloj('aceite')">🛢️ 6. Aceite (~Siglo XVI d.C.)</button>
          <button [class.active]="relojSeleccionado === 'pendulo'" (click)="seleccionarReloj('pendulo')">⚙️ 7. Péndulo (~1656)</button>
          <button [class.active]="relojSeleccionado === 'bolsillo'" (click)="seleccionarReloj('bolsillo')">⌚ 8. Bolsillo (~Siglo XVIII)</button>
          <button [class.active]="relojSeleccionado === 'cuarzo'" (click)="seleccionarReloj('cuarzo')">📟 9. Cuarzo (~1927)</button>
          <button [class.active]="relojSeleccionado === 'atomico'" (click)="seleccionarReloj('atomico')">⚛️ 10. Atómico (~1967)</button>
        </div>
      </header>
      ...
      <main class="timeline-display">
        ...
        <app-reloj-incienso *ngIf="relojSeleccionado === 'incienso'" [hora]="horaGlobal"></app-reloj-incienso>
        <app-reloj-aceite   *ngIf="relojSeleccionado === 'aceite'"   [hora]="horaGlobal"></app-reloj-aceite>
        <app-reloj-bolsillo *ngIf="relojSeleccionado === 'bolsillo'" [hora]="horaGlobal"></app-reloj-bolsillo>
        <app-reloj-atomico  *ngIf="relojSeleccionado === 'atomico'"  [hora]="horaGlobal"></app-reloj-atomico>
      </main>
    </div>
  `,
  styles: [`
    :host { display: block; background-color: #121212; color: #ffffff; min-height: 100vh; font-family: system-ui, sans-serif; }
    .dashboard-container { padding: 2rem; max-width: 1200px; margin: 0 auto; }
    header { text-align: center; margin-bottom: 2rem; }
    
    /* Credencial de Usuario Autenticado */
    .user-badge { text-align: right; color: #ffd700; font-weight: bold; margin-bottom: 1rem; font-size: 0.9rem; }
    .logout { color: #ff5555; cursor: pointer; text-decoration: underline; margin-left: 8px; }
    .logout:hover { color: #ff3333; }

    /* Panel del Slider del Tiempo */
    .slider-box { background: #1e1e1e; padding: 1.5rem; border-radius: 12px; display: inline-block; width: 100%; max-width: 550px; box-shadow: 0 4px 15px rgba(0,0,0,0.5); box-sizing: border-box; margin-bottom: 2rem; }
    ...
    .btn-play { background: #2e7d32; color: white; border: none; padding: 0.6rem 1rem; border-radius: 6px; cursor: pointer; font-weight: bold; min-width: 160px; }
    ...
    .velocidad-box { margin-top: 1rem; display: flex; justify-content: center; align-items: center; gap: 10px; }
    .velocidad-box select { background: #2a2a2a; color: white; border: 1px solid #444; padding: 6px 12px; border-radius: 4px; font-weight: bold; cursor: pointer; }
    .alert-text { display: block; font-style: italic; margin-top: 0.5rem; font-size: 0.85rem; color: #58a6ff; }
    .alert-text.manual { color: #ffa500; }
    
    /* Botonera de la Línea Temporal */
    .selector-relojes { display: flex; justify-content: center; gap: 0.5rem; margin-top: 1rem; flex-wrap: wrap; }
    .selector-relojes button { background: #2a2a2a; color: #fff; border: 1px solid #444; padding: 0.6rem 1.2rem; border-radius: 25px; cursor: pointer; transition: all 0.3s ease; font-size: 0.85rem; }
    ...
    .timeline-display { display: flex; justify-content: center; align-items: center; min-height: 420px; margin-top: 2rem; }
  `]
})
export class AppComponent implements OnInit, OnDestroy {
  ...
  // Variables del Motor de Frecuencia Cuántica
  ...
  ngOnInit() {
    // Comprobar persistencia local del laboratorio
  ...
  // Bucle de Alta Frecuencia (20 Ticks por segundo - Delta Time Sincronizado)
  arrancarBucleFrecuencia() {
    ...
    this.loopTimerId = setInterval(() => {
      ...
      if (this.enTiempoReal) {
        ...
        // Modo Manual: Avanza milisegundos para evitar la congelación matemática de los hilos visuales
      ...
      // Forzar ciclo de actualización de vista en Angular nativo
      this.cdr.detectChanges(); 
    }, 50);
  }

  // Interacción del Usuario con la Línea de Tiempo Manual
  usuarioMueveSlider() {
  ...
  // Gestión de Pasarela de Autenticación
